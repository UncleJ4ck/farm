...
nmap showed 22, 80, and 3000. Gitea on 3000 gave me the whole codebase. The app has subdomains including `admin.microblog.htb`. Registering and reading the source showed the admin edit handler manipulates an `order.txt` file by id with no sanitization:
...
The trailing `a/b` satisfies the `(.*)/(.*)` capture. With pro features unlocked, I used the same `id` traversal to write a PHP webshell:
