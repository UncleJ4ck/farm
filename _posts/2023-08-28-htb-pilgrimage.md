...
Decoding the hex from the verbose output gave the SQLite file, which held the users table.
...
`malwarescan.sh` ran `binwalk -e` on every new file in `shrunk/`.

```bash
binout="$(/usr/local/bin/binwalk -e "$filename")"
```

The binwalk version was 2.3.2, vulnerable to CVE-2022-4510, a path-traversal RCE in the PFS extractor. A crafted file makes binwalk write a malicious plugin into `~/.config/binwalk/plugins/` and execute it during extraction. I built the malicious PNG with a public PoC and dropped it into the watched directory.
