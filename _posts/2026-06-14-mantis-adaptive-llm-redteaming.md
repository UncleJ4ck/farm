...
## three follow-ups, and what each one is actually worth

After v4.0 shipped I had three loose ends. None of them is a headline. Two fall straight out of the false-positive bug above, and one is a cost knob I had been putting off. I am writing them down with their price tags attached, because a feature without a measured cost is just a story.

**The off-topic gate, a cost knob that needs a goal to do anything.** TAP's tree search spends a model call on every candidate branch, including the ones that have wandered off the actual objective. So `--prune-off-topic` adds a cheap pre-check: before paying for a candidate, ask the attacker model whether the branch is still pursuing the stated goal, and drop the ones it is confident have drifted. It is off by default and byte-identical to the old path when off. It fails open, only a confident OFF_TOPIC prunes, and a hedged or errored or empty answer keeps the candidate, because a gate that can starve the search is worse than the waste it removes. Obfuscation counts as on-topic on purpose: a jailbreak that hides its goal behind roleplay or translation is exactly the thing you do not want pruned for looking innocent.

Two honest caveats, and the second is the one that bites. First, I validated this on query count, not on break rate. It is a cost optimization, and it can cost you a break: an obfuscated-but-valid jailbreak can still read as drifted to the gate and get cut before it ever runs. Saving calls and losing a finding is a bad trade, which is why it is opt-in. Second, and worse for anyone who flips it on expecting magic, it does nothing on the stock suite. The relevance check needs a per-test attack goal, the stock category tests do not carry one, so the gate short-circuits to keep-everything before it even spends the check. Give it a goal and it does fire, but then there is a floor cost: if the attacker stays on topic the gate pays a relevance call per candidate only to keep all of them. I mapped both behaviors by running it live against Opus 4.8 and watching the break rate not move a single point in either direction. The gate works, but only if you feed it a goal, and nothing in the stock config does.

**Category memory, repurposed instead of deleted.** The v4.0 false positive came from memory keyed by category: a payload that solved one test got replayed verbatim against a different test in the same category and the judge waved it through. The fix above keyed verbatim replay to the specific test. But that left the category signal on the floor, and there is a real signal there. The technique that broke a sibling test is often the right opening move even when the exact words are wrong. So category scope is not gone, it is demoted from replay to example. Under `--memory-cells` you now get two scopes off the same store: test name replays a confirmed framing verbatim against the same test, and category feeds a confirmed framing to the attacker as a few-shot exemplar with an explicit "adapt the technique, do not copy it." The distinction is the whole point. An exemplar-derived variant is a new payload, so it still has to clear the two-evaluator judge on its own merits. The cross-test signal survives. The cross-test false positive does not, because nothing gets credited without earning its own verdict.

**Golden Rule 2, an absolute rule that fought its own orders.** The attacker's system prompt carried a flat rule: never directly ask for the harmful thing, always come in sideways. Good default, wrong on exactly one turn. Some strategy directives, the persuasion-cascade extraction step in particular, explicitly tell the attacker to stop hedging and consolidate everything into one directly-usable artifact. That is the turn that is supposed to close the break, and the absolute "never ask directly" was quietly smothering it. The attacker hedged on the one turn it was told to commit. So I de-absolutized the rule: it keeps the indirect default everywhere, plus an explicit exception, plus a per-turn override clause that renders only when a directive actually demands the extraction. Indirect by default, direct when and only when ordered to be.

The caveat here is sharper than the other two, because I cannot show you the payoff. I verified the fix at the level of the prompt: a discriminator drives the real prompt builder and asserts the absolute rule is gone and the override appears only when a directive is present, and is absent on a plain turn. That part is solid. What I cannot demonstrate is the behavioral win, that the relaxed turn lands a break the old wording would have suppressed. The attacker model I run these checks against is Opus 4.8, and Opus refuses the attacker role on these goals, so it returns an empty payload rather than a more direct one. I can prove the prompt changed exactly as intended. I cannot, with this attacker, prove the change earns its keep. That is an honest half-result, and I would rather log it as half than dress it up as whole.
